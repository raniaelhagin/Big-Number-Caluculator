def add(op1, op2):
    """
    Adds two operands and returns the result of the summation

    Parameters:
        op1 (int): the first operand to be added 
        op2 (int): the second operand to be added  
    
    Returns:
        res (int): the result of the summation
    """
...
def subtract(op1, op2):
    """
    Subtracts two operands and returns the result of the summation
    ...
    Returns:
        res (int): the result of the subtraction
    """
    op1_digits = [int(digit) for digit in op1][::-1]
    op2_digits = [int(digit) for digit in op2][::-1]

    # check if op2 is greater than op1 and set negative to true and swap
    negative = False
    if len(op2_digits) > len(op1_digits):
        negative = True
        op1_digits, op2_digits = op2_digits, op1_digits
...
for query in queries:
    ...
    if operator == "SQRT" or operator == "ABS":
        ...
        if operator == "ADD":
            ...
            elif is_neg_op1 and (not is_neg_op2):
                ans = subtract(operand2, operand1[1:])
            elif is_neg_op2 and (not is_neg_op1):
